...
def get_prediction(output_knn, Y_train, X_test, X_train):
    pcc = []
    neighbours_sign = []
    for ik in output_knn:
        neighbours_sign.append(sign(float(Y_train[ik])))

        # get PCC value
        v1 = np.asarray(X_train[:, ik], dtype='float')
        v2 = np.asarray(X_test, dtype='float')
        pcc_val = sp.stats.pearsonr(v1, v2)[0]
        pcc.append(pcc_val)
    score = np.dot(neighbours_sign, pcc)
    # num_1 = np.sum(neighbours_sign)
    # avg = np.mean(neighbours_sign)

    return score

d = pd.read_csv('DREAM_data.csv', header=None)
data = d.values
...
# for jx in range(1):
    ...
    for ix in range(cell_data.shape[1]):
        ...
        # print preds_index
        pred = get_prediction(preds_index, y_train, x_test, x_train)
        # print pred
